...
export const Lyrics = () => {
  ...
  const { data: videos, mutate } = useSWR([ENDPOINTS.USER_LYRICS, API_METHODS.GET], ([url, method]) => request<Video[]>(method, url));
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid w-full gap-4 grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 place-items-center md:place-items-start">
        {videos?.map((video) => (
          <CardVideo key={video.uuid} video={video} onDelete={() => handleDeleteLyric(video.uuid)} onEdit={() => {}} />
        ))}
      </div>
    </div>
  );
