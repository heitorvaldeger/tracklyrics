...
test.group('VideoFindService.findBy()', () => {
  test('it must returns a empty list of videos with on success', async ({ expect }) => {
    const { sut } = makeSut()
  ...
  test('it must returns a list videos returns on find by genre', async ({ expect }) => {
    const { sut } = makeSut()
  ...
  test('it must returns a list videos returns on find by language', async ({ expect }) => {
    const { sut } = makeSut()
  ...
  test('it must returns a list videos returns on find by user uuid', async ({ expect }) => {
    const { sut } = makeSut()
    const video = await sut.findBy({
